...
list(set(countries_rest).difference(countries_bank))
...
#
# #### Methodology
# a) contrast the amount of debt cancellation received with the mean CPIA debt policy rating for each country in a x-y plot
#
# b) evaluate the correlation of the debt cancellation time series with the CPIA rating time series (Pearson coefficient)
...
###############################################################################
# The average correlation for a country is mildly positive (i.e. amt of debt cancellation increases, debt policy rating increases). This is not what was expected in light of the criteria for setting the CPIA rating explained above. However, the interpretation of the mean becomes less expressive in light of the significant standard deviation. An interesting observation is also that the results appear to cluster in three regions: little correlation, strong negative correlation (smaller than -0.5), strong positive correlation (larger than 0.5). This could be coincidence, or that Chinese debt cancellation simply doesn't matter much in the CPIA process, but it could also hint at a third factor that is at play, possibly related to political orientation, which may decide whether debt cancellation by China influences the CPIA rating to the positive or to the negative.
#
# #### Limitations of the method and how to improve on it
# The temporal resolution of this method is not very granular. It seems intuitive that a case of debt cancellation has the most influence on the CPIA rating in the years surrounding the event. However, the Pearson correlation coefficient considers a very long time horizon. For example, if a country receives debt cancellation in 2003, and the CPIA rating increases in 2017, the Pearson coefficient would still capture this and be tilted towards a positive value, even though it seems unlikely in practice that a debt cancellation from 2003 influences the CPIA rating in 2017. This could be mitigated by extracting time intervals surrounding the events of debt cancellation by a few years, and then calculating the correlation on that. In addition, as mentioned previously, an isolation of the effect of debt cancellation on the CPIA rating by controlling for other variables that may affect the debt rating would also be desirable.

###############################
# ### Relation between amount of debt cancellation and number of government visits
...
# Intersection of the country coverage
list(set(countries_canc) & set(countries_govt))
